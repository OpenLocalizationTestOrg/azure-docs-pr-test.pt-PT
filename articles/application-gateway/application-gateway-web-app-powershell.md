...
Gateway de aplicação permite-lhe toohave uma aplicação Web do Azure ou outro serviço de multi-inquilino como um membro do conjunto back-end. Neste artigo, tooyou saiba tooconfigure uma aplicação web do Azure com o Gateway de aplicação. primeiro exemplo de Olá mostra-lhe como tooconfigure um toouse de gateway de aplicação numa aplicação web como um membro do conjunto back-end existente. segundo exemplo de Olá mostra-lhe como toocreate um novo gateway de aplicação com uma aplicação web como um membro do conjunto back-end.
...
Olá exemplo seguinte adiciona uma aplicação web como um gateway de aplicação existentes do conjunto back-end membro tooan. Ambos Olá comutador `-PickHostNamefromBackendHttpSettings`na configuração de pesquisa de Olá e `-PickHostNameFromBackendAddress` de http de back-end Olá definições tem de ser fornecidas por ordem para toowork de aplicações web.
...
Este cenário implementa uma aplicação web com o asp.net Olá obter Web site tiver sido iniciado e um gateway de aplicação.
...
Assim que for criado o gateway de Olá, o passo seguinte Olá é tooconfigure Olá front-end para comunicação. Ao utilizar um IP público, o gateway de aplicação requer um nome DNS dinamicamente atribuído, que não é amigável. os utilizadores finais de tooensure pode atingiu o gateway de aplicação Olá, um registo CNAME pode ser utilizado toopoint toohello ponto final público Olá do gateway de aplicação. Olá toocreate alias, obter detalhes de Olá do gateway de aplicação Olá e o respetivo nome IP/DNS associado com o gateway de aplicação do Olá PublicIPAddress elemento toohello anexado. Isto pode ser feito com o DNS do Azure ou de outros fornecedores DNS, por criar um registo CNAME que aponta toohello [endereço IP público](../dns/dns-custom-domain.md#public-ip-address). utilização de Olá de registos não é recomendada, uma vez que podem ser alterados Olá VIP no momento do reinício do gateway de aplicação.
...
Saiba como tooconfigure redirecionamento, visitando: [configurar redirecionamento no Gateway de aplicação com o PowerShell](application-gateway-configure-redirect-powershell.md).
