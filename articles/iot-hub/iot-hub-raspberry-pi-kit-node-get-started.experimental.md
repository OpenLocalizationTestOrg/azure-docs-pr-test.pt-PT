...
Não tem um kit ainda? Tente Olá [Raspberry Pi 3 emulador](https://blogs.msdn.microsoft.com/iliast/2016/11/10/how-to-emulate-raspberry-pi/). Ou comprar um novo kit [aqui](https://azure.microsoft.com/develop/iot/starter-kits).
...
1. Transferir Raspbian.
   1. [Transferir Raspbian Jessie com Pixel](https://www.raspberrypi.org/downloads/raspbian/) (ficheiro. zip de Olá).
...
Utilize Olá de breadboard e jumper de tooconnect de cablagem um LED e um tooPi BME280 da seguinte forma. Se não tiver sensor Olá, ignore esta secção.
...
| SCK (Pin 8G)             | I2C1 SDA (Pin 3)       | Cabo laranja  |
| SDI (Pin 10g)            | I2C1 SCL (Pin 5)       | Cabo vermelho     |
...
1. Utilize um dos Olá os seguintes clientes SSH do seu tooyour de tooconnect do computador anfitrião Raspberry Pi.
    - [PuTTY](http://www.putty.org/) para Windows. Terá de endereço IP de Olá do seu tooconnect Pi-lo através de SSH.
    - Olá SSH no cliente incorporado Ubuntu ou macOS. Poderá precisar de executar a `ssh pi@<ip address of pi>` tooconnect Pi através de SSH.
...
1. Instale todos os pacotes hello os seguintes comandos. Inclui dispositivos IoT do Azure SDK, BME280 Sensor biblioteca e da biblioteca de preparar o Pi.
   ...
   ```
   > [!NOTE] 
   Poderá demorar vários minutos toofinish este denpening do processo de instalação na sua ligação de rede.
...
1. Abra o ficheiro de configuração de Olá executando Olá os seguintes comandos:
   ...
   Existem dois itens neste ficheiro, pode configurate. Olá primeiro um é `interval`, que define o intervalo de tempo de Olá entre duas mensagens que enviam toocloud. Olá segunda `simulatedData`, que é um valor booleano para se toouse simulated dados de sensores ou não.
