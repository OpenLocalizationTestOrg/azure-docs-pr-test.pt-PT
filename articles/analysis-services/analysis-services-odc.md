...
1. Copie o ficheiro de ligação de exemplo de Olá abaixo e colar no editor de texto. 

2. No `odc:ConnectionString`, alterar Olá seguintes propriedades:

    *   No `Data Source=asazure://<region>.asazure.windows.net/<servername>;` alterar `<region>` toohello região do seu servidor de Analysis Services e `<servername>` toohello nome do seu servidor.

    *   No `Initial Catalog=<database>;` alterar `<database>` toohello nome da base de dados.

3. No `<odc:CommandText>Model</odc:CommandText>` alterar `Model` toohello nome do modelo ou perspetiva. 

4. Guarde o ficheiro de Olá com um `.odc` extensão toohello C:\Users\\*username*\Documents\My pasta de origens de dados.

5. Clique no ficheiro de Olá e, em seguida, clique em **abrir no Excel**. Ou no Excel, no Olá **dados** do Friso, clique em **as ligações existentes**, selecione o ficheiro e, em seguida, clique em **abra**.
