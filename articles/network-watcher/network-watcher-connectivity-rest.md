...
---
# <a name="check-connectivity-with-azure-network-watcher-using-hello-azure-portal"></a>Verifique a conectividade com o observador de rede do Azure utilizando Olá portal do Azure
...
Saiba como toouse conectividade tooverify se uma ligação de TCP direta de um tooa de máquina virtual fornecido ponto final pode ser estabelecida.

## <a name="before-you-begin"></a>Antes de começar

Este artigo pressupõe que tem Olá os seguintes recursos:

* Uma instância de observador de rede na região de Olá que pretende toocheck conectividade.

* Máquinas virtuais toocheck conectividade.
...
## <a name="register-hello-preview-capability"></a>Registar a capacidade de pré-visualização de Olá

Verificação da conectividade está, atualmente em pré-visualização pública, toouse esta funcionalidade necessita de toobe registado. toodo, Olá de execução do PowerShell exemplo a seguir:
...
o registo de Olá tooverify foi bem sucedido, execute Olá Powershell de exemplo a seguir:

```powershell
Get-AzureRmProviderFeature -FeatureName AllowNetworkWatcherConnectivityCheck  -ProviderNamespace  Microsoft.Network
```

Se a funcionalidade de Olá foi registada correctamente, saída Olá deve corresponder ao seguinte Olá:
...
Execute Olá tooreturn de script a seguir uma máquina virtual. Estas informações são necessárias para a execução de conectividade. 

Olá seguinte código tem valores para Olá seguintes variáveis:

- **subscriptionId** -Olá toouse de ID de subscrição.
- **resourceGroupName** - hello nome de um grupo de recursos que contém máquinas virtuais.
...
Do seguinte de Olá de saída, Olá ID da máquina virtual de Olá é utilizado no seguinte exemplo de Olá:
...
## <a name="check-connectivity-tooa-virtual-machine"></a>Verifique a conectividade tooa máquina

Neste exemplo verifica conectividade tooa máquina de virtual de destino através da porta 80.
...
Uma vez que esta operação é muito a executar, Olá resultado Olá é devolvido o URI no cabeçalho de resposta de Olá conforme mostrado no Olá seguinte resposta:
...
Olá seguinte resposta é do exemplo anterior Olá.  Esta resposta, Olá `ConnectionStatus` é **Unreachable**. Pode ver todos os Olá de sondas enviadas com falhas. Falha de conectividade de Olá na aplicação virtual Olá tooa devida configurada pelo utilizador `NetworkSecurityRule` denominado **UserRule_Port80**, configurado tooblock tráfego de entrada na porta 80. Estas informações podem ser utilizados tooresearch problemas de ligação.
...
exemplo de Olá verifica a conectividade entre uma máquina virtual e um ponto final remoto.
...
No seguinte exemplo de Olá, Olá `connectionStatus` é mostrado como **Unreachable**. No Olá `hops` mais detalhes, pode ver em `issues` que tráfego Olá foi bloqueado devido tooa `UserDefinedRoute`.
...
Olá exemplo seguinte verifica o Web site do Olá conectividade tooa.
...
Olá resposta a seguir, pode ver Olá `connectionStatus` mostra como **Reachable**. Quando uma ligação é bem-sucedida, são fornecidos valores de latência.
...
## <a name="check-connectivity-tooa-storage-endpoint"></a>Verifique o ponto final de armazenamento de tooa de conectividade

Olá exemplo seguinte verifica a conetividade de Olá de uma conta de armazenamento do blogue de tooa máquina virtual.
...
Olá exemplo seguinte é resposta Olá a execução de chamada de API Olá anterior. Como a verificação de Olá for bem sucedida, Olá `connectionStatus` propriedade mostra como **Reachable**.  São fornecidos detalhes de Olá sobre o número de Olá de blob de armazenamento do saltos tooreach necessário Olá e latência.
...
Saiba como capturas de pacotes de tooautomate com alertas de Máquina Virtual visualizando [criar uma captura de pacotes accionadas alerta](network-watcher-alert-triggered-packet-capture.md)
