...
Pode ligar remotamente tooyour dispositivo através do Windows PowerShell. Quando se liga desta forma, não verá um menu. (Pode ver um menu apenas se utilizar a consola de série de Olá no Olá dispositivo tooconnect.) Com comunicação remota do Windows PowerShell, ligar específico tooa de espaço de execução. Também pode especificar o idioma de apresentação de Olá.

Para mais informações sobre como utilizar o seu dispositivo toomanage de comunicação remota do Windows PowerShell, visite demasiado[utilize o Windows PowerShell para StorSimple tooadminister o dispositivo StorSimple](storsimple-8000-windows-powershell-administration.md).

Este tutorial explica como tooconfigure o dispositivo para a gestão remota e, em seguida, tooconnect tooWindows do PowerShell para StorSimple. Pode utilizar HTTP ou HTTPS tooremotely estabelecerem ligação através do Windows PowerShell. No entanto, quando decidir como tooconnect tooWindows do PowerShell para StorSimple, considere Olá seguintes informações:
...
Pode utilizar qualquer um dos Olá portal ou Olá consola de série tooconfigure gestão remota do Azure. Selecione uma das Olá os seguintes procedimentos:

* [Utilize a gestão remota do Olá tooenable portal do Azure através de HTTP](#use-the-azure-classic-portal-to-enable-remote-management-over-http)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-http"></a>Utilize a gestão remota do Olá tooenable portal do Azure através de HTTP

Efetue Olá seguindo os passos na gestão remota do Olá tooenable portal do Azure através de HTTP.

#### <a name="tooenable-remote-management-through-hello-azure-portal"></a>gestão remota do tooenable através do Olá portal do Azure

1. Aceda tooyour do serviço StorSimple Manager de dispositivo. Selecione **dispositivos** e, em seguida, selecione e clique em dispositivo Olá tooconfigure que pretende para a gestão remota. Aceda demasiado**definições do dispositivo > segurança**.
2. No Olá **definições de segurança** painel, clique em **gestão remota**.
3. No Olá **gestão remota** painel, defina **ativar a gestão remota** demasiado**Sim**.
...
* [Utilize a gestão remota do Olá tooenable portal do Azure através de HTTPS](#use-the-azure-classic-portal-to-enable-remote-management-over-https)
...
### <a name="use-hello-azure-portal-tooenable-remote-management-over-https"></a>Utilize a gestão remota do Olá tooenable portal do Azure através de HTTPS

Efetue Olá seguindo os passos na gestão remota do Olá tooenable portal do Azure através de HTTPS.

#### <a name="tooenable-remote-management-over-https-from-hello-azure-portal"></a>gestão remota tooenable através de HTTPS de Olá portal do Azure
...
5. Clique em... e, em seguida, clique em **Transferir certificado de gestão remoto**. Especifique uma localização toosave este ficheiro. É necessário tooinstall este certificado no computador de cliente ou anfitrião Olá que irá utilizar tooconnect toohello dispositivo.
...
Cada um dos Olá precedente procedimentos, é descrito abaixo.

#### <a name="tooimport-hello-certificate-on-hello-remote-host"></a>certificado de Olá tooimport no anfitrião remoto Olá
1. Clique no ficheiro. cer de Olá e selecione **instalar certificado**. Esta ação inicia Olá Assistente para importar certificados.
...
Utilizar o Windows PowerShell e do SSL tooenter uma sessão de SSAdmin no seu dispositivo a partir de um anfitrião remoto ou o cliente. sessão de SSAdmin Olá mapeia toooption 1 na Olá [consola de série](storsimple-8000-windows-powershell-administration.md#connect-to-windows-powershell-for-storsimple-via-the-device-serial-console) menu do seu dispositivo.
...
2. Adicione anfitriões fidedignos Olá dispositivo IP endereço toohello do cliente, escrevendo:
    ...
    Onde <*device_ip*> é o endereço IP Olá do seu dispositivo; por exemplo: 
     ...
     `Set-Item wsman:\localhost\Client\TrustedHosts 10.126.173.90 -Concatenate -Force`
3. toocreate uma credencial nova, escreva:
...
* Saiba mais sobre [através do Windows PowerShell tooadminister o dispositivo StorSimple](storsimple-8000-windows-powershell-administration.md).
