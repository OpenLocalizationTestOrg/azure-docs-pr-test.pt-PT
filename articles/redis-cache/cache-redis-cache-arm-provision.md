...
Neste tópico, saiba como toocreate um modelo Azure Resource Manager que implementa do Azure a Cache de Redis. cache de Olá pode ser utilizado com um armazenamento conta tookeep diagnóstico dados existentes. Também saber como toodefine os recursos são implementados e como toodefine parâmetros que são especificados quando a implementação de Olá é executada. Pode utilizar este modelo para as suas próprias implementações ou personalize-toomeet os seus requisitos.

Atualmente, definições de diagnóstico são partilhadas para todas as caches no Olá mesma região para uma subscrição. Atualizar uma cache na região de Olá afeta todos os outros caches na região de Olá.
...
Para o modelo completo Olá, consulte [modelo de Cache de Redis](https://github.com/Azure/azure-quickstart-templates/blob/master/101-redis-cache/azuredeploy.json).

> [!NOTE]
> Modelos do Resource Manager para Olá novo [escalão Premium](cache-premium-tier-intro.md) estão disponíveis. 
> 
...
> 
> toocheck para modelos de Olá mais recentes, consulte [modelos de início rápido do Azure](https://azure.microsoft.com/documentation/templates/) e procure `Redis Cache`.
> 
> 
...
localização Olá Olá a Cache de Redis. Para melhor desempenho, utilize Olá mesma localização como Olá toobe aplicações utilizada com a cache de Olá.
...
nome de Olá do Olá toouse de conta de armazenamento existente para obter um diagnóstico. 
...
Um valor booleano que indica se tooallow aceder através de portas não SSL.
...
Cria Olá a Cache de Redis do Azure.
