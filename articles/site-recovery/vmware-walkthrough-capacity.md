...
---
# <a name="step-3-plan-capacity-and-scaling-for-vmware-tooazure-replication"></a>Passo 3: Planear a capacidade e Dimensionar para replicação de tooAzure de VMware
...
Recolher informações sobre o ambiente de replicação e, em seguida, planear a capacidade de utilizar esta informação, juntamente com considerações de Olá realçado neste artigo.


## <a name="gather-information"></a>Reunir informações

1. Transferir Olá [ferramenta de implementação Planner](https://aka.ms/asr-deployment-planner) para replicação de VMware.
2. [Leia este artigo](site-recovery-deployment-planner.md) toounderstand como toorun Olá ferramenta.
3. Utilizando a ferramenta de Olá recolher informações sobre compatíveis e incompatíveis VMs, discos por VM, e dados churn por disco. ferramenta Olá também abrange os requisitos de largura de banda de rede e Olá necessária para ativação pós-falha com êxito de replicação e teste de infraestrutura do Azure.

## <a name="replication-considerations"></a>Considerações sobre replicação

Antes de começar a implementação, tenha em atenção estes considerações:
...
Siga estes tooset instruções configurar um servidor de processos adicionais. Depois de configurar o servidor de Olá, migrar toouse de máquinas de origem-lo.

1. No **servidores do Site Recovery**, clique em servidor de configuração de Olá > **+ o servidor de processos**.
...
3. Transfira o ficheiro de configuração do Site Recovery Unified Olá.
4. Executar o servidor de processos do programa de configuração tooinstall Olá e registá-lo no Cofre de Olá.
5. No **antes de começar**, selecione **adicionar tooscale de servidores de processos adicionais saída implementação**.
6. No **detalhes do servidor de configuração**, especifique o endereço IP de hello do servidor de configuração de Olá e Olá o frase de acesso. Se não tiver o frase de acesso de Olá, obtenha-o executando **[SiteRecoveryInstallationFolder]\home\sysystems\bin\genpassphrase.exe – n** no servidor de configuração de Olá.

    ![Servidor de configuração](./media/vmware-walkthrough-capacity/add-ps2.png)
7. Conclua o restante Olá configuração Olá mesma forma que fez quando configurou o servidor de configuração de Olá.

### <a name="migrate-machines-toouse-hello-process-server"></a>Migrar servidor de processos do máquinas toouse Olá
...
Aceda demasiado[passo 4: planear o funcionamento em rede](vmware-walkthrough-network.md).
